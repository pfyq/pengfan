...
//                 $("#password").val(hex_md5($("input[name='password']").val()));
        ...
        $(function() {
        	if(""=="1"){
        		layer.alert("<font color='black'>用户名或登录密码不正确，请重新输入。您还可以：<a href='/static/platform/jsp/backPass/400NumCenter.jsp'>重置登录密码</a></font>");
        	}else if(""=="2"){
        		layer.alert("<font color='black'>用户名或登录密码不正确，还有3次机会。您还可以：<a href='/static/platform/jsp/backPass/400NumCenter.jsp'>重置登录密码</a></font>");
        	}else if(""=="3"){
        		layer.alert("<font color='black'>用户名或登录密码不正确，还有2次机会。您还可以：<a href='/static/platform/jsp/backPass/400NumCenter.jsp'>重置登录密码</a></font>");
        	}else if(""=="4"){
        		layer.alert("<font color='black'>用户名或登录密码不正确，还有1次机会。您还可以：<a href='/static/platform/jsp/backPass/400NumCenter.jsp'>重置登录密码</a></font>");
        	}else if("">="5"){
        		layer.alert("<font color='black'>登录密码出错已达上限将锁定登录3小时,请<a href='/static/platform/jsp/backPass/400NumCenter.jsp'>重置登录密码</a>后登录!</font>");
        	}else{
	            if (""=="simplePassword") {
	                var win = $("#cpws").window({
						title 	: "修改密码",
